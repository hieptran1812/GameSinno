...
const shuffleDirections = (directions) => {
  let currentIdx = directions.length,
    tempVal,
    randomIdx;
...
const randomStartAndEnd = (nodeList) => {
  ...
  const startNode = nodeList[startNodeIdx],
    endNode = nodeList[endNodeIdx];
...
const checkSolution = (maze, start, end, destructuredSolution) => {
  ...
  return lastValidPosition[0] === end[0] && lastValidPosition[1] === end[1]
    ? { success: pathTaken }
    : { incomplete: pathTaken };
};
...
const destructureSolution = (solution) => {
  ...
  for (let element of solution) {
    ...
    if (!DIR_SET.has(cleanedElement)) {
      const loopArr = cleanedElement
        .substring(5, cleanedElement.length - 1)
        .split("-");
